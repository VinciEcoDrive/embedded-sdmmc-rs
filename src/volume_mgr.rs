    ...
    <D as BlockDevice>::Error: core::fmt::Debug,
{
    ...
    /// Same as `close_file`, but doesn't update the directory entry.
    pub fn on_file_dropped(&mut self, file: RawFile) -> Result<(), Error<D::Error>> {
        let mut found_idx = None;
        for (idx, info) in self.open_files.iter().enumerate() {
            if file == info.file_id {
                found_idx = Some((info, idx));
        ...
        if let Some((file_info, file_idx)) = found_idx {
            if file_info.dirty {
                crate::warn!(
                    "File was dropped without being closed, and was dirty. This is a bug."
                );
            }
            self.open_files.swap_remove(file_idx);
        }
